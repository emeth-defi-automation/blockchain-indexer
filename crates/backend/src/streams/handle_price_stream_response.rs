...
pub async fn handle_price_stream_response(
    ...
    match result {
        Ok(Message::Text(text)) => {
            ...
            if kline.data.close_time
                != *current_close_time
                    .get(&kline.data.symbol)
                    .expect("There will always be a current close time")
            {
                let new_record = add_token_price_history_record(kline.data.clone()).await?;
                current_close_time.insert(
                    kline.data.clone().symbol,
                    new_record.timestamp.timestamp_millis() as u64,
                );
                ...
                let changed_record = update_token_price_history_record(
                    record_id.get(&kline.data.symbol).unwrap().clone(),
                    kline.data.clone(),
                )
                .await?
                ...
                current_close_time.insert(
                    kline.data.clone().symbol,
                    changed_record.timestamp.timestamp_millis() as u64,
                );
